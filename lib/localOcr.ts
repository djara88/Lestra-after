...
function dateFromText(text: string): Date | null {
  ...
  if (numeric) {
    ...
    if (!Number.isNaN(d.getTime()) && d.getDate() === day && d.getMonth() === month) {
      if (!numeric[3] && d.getTime() < Date.now() - 86_400_000) d.setFullYear(d.getFullYear() + 1);
      return d;
    }
  }
  ...
  if (named) {
    ...
    const d = new Date(year, month, day, 18, 0, 0, 0);
    if (!named[3] && d.getTime() < Date.now() - 86_400_000) d.setFullYear(d.getFullYear() + 1);
    return d;
  }

  const normalized = stripAccents(text.toLowerCase());
  const weekdays: Array<[string, number]> = [['domingo',0],['lunes',1],['martes',2],['miercoles',3],['jueves',4],['viernes',5],['sabado',6]];
  for (const [word, day] of weekdays) if (new RegExp(`\\b${word}\\b`).test(normalized)) return nextWeekday(day);
  if (/\bma[nñ]ana\b/i.test(text)) { const d = new Date(); d.setDate(d.getDate()+1); d.setHours(18,0,0,0); return d; }
  return null;
}
...
export function parseSchoolText(rawText: string): LocalCandidate[] {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (academicType) {
      ...
      if (!seen.has(key)) {
        seen.add(key);
        candidates.push({
          candidate_type: 'academic_item', title, description: context === line ? null : context,
          due_at: when, confidence: when ? 0.88 : 0.76, academic_type: academicType,
          subject, priority: academicType === 'test' || academicType === 'exam' ? 'high' : 'normal', materials,
        });
      }
      continue;
    }
    ...
    for (const [pattern, category] of EVENT_KEYWORDS) if (pattern.test(line)) { eventCategory = category; break; }
    if (eventCategory) {
      const title = titleFromLine(line, 'Actividad del colegio');
      const key = `e:${title.toLowerCase()}:${when ?? ''}`;
      if (!seen.has(key)) {
        seen.add(key);
        candidates.push({ candidate_type: 'calendar_event', title, description: context === line ? null : context, starts_at: when, confidence: when ? 0.86 : 0.7, subject: eventCategory });
      }
    }
  }
  ...
  if (candidates.length === 0) {
    ...
    const when = applyTime(dateFromText(text), text);
    const materials = materialsFromText(text);
    candidates.push({
      candidate_type: 'note',
      title: subject ? `Aviso de ${subject}` : 'Aviso del colegio',
...
async function ocrImage(uri: string) {
  if (!isSupported()) throw new Error('Este dispositivo no admite OCR local.');
...
export async function runLocalOcr(uri: string, mimeType: string): Promise<LocalOcrResult> {
  ...
  if (mimeType === 'application/pdf') {
    ...
    const pages = await convertPages(uri, 0, lastPage, { format: 'jpeg', maxWidth: 2200, quality: 0.9, output: 'file' });
  ...
  if (!text) throw new Error('No encontramos texto legible en el archivo. Intenta con una foto más clara.');
  return { text, candidates: parseSchoolText(text), pageCount, processedPages, provider: 'on-device-mlkit' };
}
