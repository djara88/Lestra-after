...
export default async function PlatformPage() {
  ...
  return <main className="shell">
    ...
    <section className="panel"><h2>Familias / cuentas</h2><div className="tableWrap"><table><thead><tr><th>Familia</th><th>Estado</th><th>Miembros</th><th>Alumnos</th><th>Alta</th></tr></thead><tbody>{rows.map(r=><tr key={r.id}><td><strong>{r.name}</strong></td><td>{r.status}</td><td>{r.members}</td><td>{r.students}</td><td>{new Date(r.created_at).toLocaleDateString('es-CL')}</td></tr>)}</tbody></table></div></section>
