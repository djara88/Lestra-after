create table if not exists after.platform_admins (
  user_id uuid primary key references auth.users(id) on delete cascade,
  active boolean not null default true,
  created_at timestamptz not null default now()
);
alter table after.platform_admins enable row level security;
revoke all on after.platform_admins from anon, authenticated;

create or replace function after_private.is_platform_admin() returns boolean
language sql stable security definer set search_path='' as $$
  select coalesce((auth.jwt()->'app_metadata'->>'provider')='google',false)
     and exists(select 1 from after.platform_admins a where a.active and a.user_id=(select auth.uid()));
$$;
revoke all on function after_private.is_platform_admin() from public;
grant execute on function after_private.is_platform_admin() to authenticated;

create or replace function public.after_platform_admin_access() returns boolean
language sql stable security definer set search_path='' as $$ select after_private.is_platform_admin(); $$;
