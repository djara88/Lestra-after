...
export default function Family(){
  ...
  const [studentId,setStudentId]=useState(''); const [memberId,setMemberId]=useState(''); const [inviteEmail,setInviteEmail]=useState(''); const [inviteName,setInviteName]=useState('');
  ...
  async function invite(){
    if(!workspace.family_id||!/^\S+@\S+\.\S+$/.test(inviteEmail.trim()))return Alert.alert('Correo inválido','Ingresa el correo de la cuenta Google que usará la otra persona.');
    setBusy(true);const {error}=await supabase.rpc('after_invite_member',{p_family_id:workspace.family_id,p_email:inviteEmail.trim().toLowerCase(),p_display_name:inviteName.trim()||null,p_role:'parent'});setBusy(false);
    if(error)return Alert.alert('No pudimos invitar','Solo el propietario de la familia puede enviar invitaciones.');
    setInviteEmail('');setInviteName('');Alert.alert('Invitación preparada','Cuando esa persona ingrese con la misma cuenta Google podrá unirse a esta familia.');
  }
  ...
  return <SafeAreaView style={s.safe}><ScrollView contentContainerStyle={s.body} keyboardShouldPersistTaps="handled">
    ...
    {me?.role==='owner'?<View style={s.card}><Text style={s.heading}>Invitar a otro apoderado</Text><Text style={s.muted}>Usa el correo exacto de su cuenta Google. La invitación no concede acceso hasta que esa persona inicia sesión y la acepta.</Text><TextInput style={s.input} value={inviteEmail} onChangeText={setInviteEmail} autoCapitalize="none" keyboardType="email-address" maxLength={254} placeholder="correo@gmail.com"/><TextInput style={s.input} value={inviteName} onChangeText={setInviteName} maxLength={100} placeholder="Nombre (opcional)"/><Pressable disabled={busy} onPress={invite} style={[s.button,busy&&s.disabled]}><Text style={s.buttonText}>Invitar</Text></Pressable></View>:null}
