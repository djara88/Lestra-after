...
type Dashboard = { students_total: number; events_next_7_days: number; pending_academic: number; pending_responsibilities: number; upcoming: Array<{ id: string; title: string; category: string; starts_at: string; student_id?: string | null }> };
...
export default function Today() {
  ...
  return <SafeAreaView style={s.safe}><ScrollView contentContainerStyle={s.body} refreshControl={<RefreshControl refreshing={loading} onRefresh={load}/> }>
    ...
    {loading&&!dashboard?<ActivityIndicator style={{marginTop:24}}/>:<>
      <View style={s.metrics}><View style={s.metric}><Text style={s.number}>{dashboard?.pending_academic??0}</Text><Text style={s.label}>Académicos</Text></View><View style={s.metric}><Text style={s.number}>{dashboard?.pending_responsibilities??0}</Text><Text style={s.label}>Por coordinar</Text></View><View style={s.metric}><Text style={s.number}>{dashboard?.events_next_7_days??0}</Text><Text style={s.label}>Próx. 7 días</Text></View></View>
